...
class ANNBrainInstance(BrainInstance):
    ...
    def control(
        ...
        accelerated_mode = any(
            sensor_state.get_active_hinge_sensor_state(sensor).position < threshold
            for sensor in sensors
        )

        if accelerated_mode:
            target = 0.5 + 0.5 * math.sin(2 * self.total_time)
            print("accelerated")
        else:
            target = 0.5 + 0.5 * math.sin(self.total_time)
